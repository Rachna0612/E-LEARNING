...
6. [Monitoring Setup (Prometheus + Grafana)](#monitoring-setup)
7. [Verification & Access](#verification--access)
...
1. **Login to Grafana** (http://<MASTER_IP>:30030)

2. **Prometheus is already configured as data source!**
...
5. **Import Another Dashboard for Pods:**
...
**Symptom:** Can't access http://<IP>:30080
...
1. **Show Architecture Diagram** (draw on board or slide)
   ```
...
**Q: Why not deploy directly on EC2?**
A: Direct deployment doesn't provide auto-scaling, self-healing, or easy rollback. Kubernetes gives us all these features.

**Q: Why use Docker?**
A: Docker ensures the app runs the same way in development, testing, and production. "It works on my machine" problem is solved.

**Q: What if Master node fails?**
A: In production, we use multiple master nodes (HA setup). For this demo, we have one master to keep it simple.

**Q: How does Kubernetes know a pod is unhealthy?**
A: Kubernetes uses liveness and readiness probes - it sends HTTP requests to the pod. If it doesn't respond, K8s restarts it.

**Q: Can this handle 1 million users?**
A: Yes, by scaling replicas and adding more worker nodes. Kubernetes can scale to thousands of nodes.
...
1. **Use copy-paste for long commands** - Don't type manually
2. **Keep both terminals open** (Master and Worker) - Switch between them
3. **Use `kubectl get pods -w`** - Watch mode shows real-time updates
4. **Bookmark IPs** - Save Master/Worker IPs in notepad
5. **Take screenshots** - Capture working dashboards for presentation
6. **Test before presentation** - Run through demo once
...
2. **Use Ingress** - Instead of NodePort (more professional)
3. **Add CI/CD** - GitHub Actions to auto-deploy on git push
4. **Multi-Master Setup** - High availability
5. **Use Managed K8s** - EKS, GKE, or AKS (easier management)
6. **Add Logging** - ELK stack or Loki
7. **Add Alerts** - Alertmanager notifications
8. **Use Helm Charts** - Package your app as Helm chart
